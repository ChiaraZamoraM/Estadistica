...
Una muestra de datos se denomina cualitativa, también conocida como categórica, si sus valores pertenecen a una colección de clases no superpuestas definidas conocidas. Los ejemplos comunes incluyen la calificación de letras de los estudiantes (A, B, C, D o F), calificación de bonos comerciales (AAA, AAB, ...) y tallas de zapatos de ropa de consumo (1, 2, 3, ...).

Los tutoriales de esta sección se basan en un marco de datos integrado de R llamado pintores. Es una recopilación de información técnica de algunos pintores clásicos del siglo XVIII. El conjunto de datos pertenece al paquete MASS y debe cargarse previamente en el espacio de trabajo de R antes de su uso.
...
```{r}
#setwd("C:/Users/soyma/Documents/2022/Trabajo/Pulso PUCP/Bases de datos")
```

### **3. Tipos y creación de objetos**
...
Si dos vectores tienen una longitud diferente, el más corto se reciclará para que coincida con el vector más largo. 

Por ejemplo, los siguientes vectores u y v tienen diferentes longitudes y su suma se calcula reciclando los valores del vector u más corto.
```{r}
u = c(10, 20, 30) 
...
Reproducimos una representación de memoria de la matriz en R con la función de *matrix*. Los elementos de datos deben ser del mismo tipo básico.
```{r}
#?matrix()
```
...
```{r}
A = matrix( 
   c(2, 4, 3, 1, 5, 7), # elementos de la data
   nrow=2,              # número de filas
   ncol=3,              # número de columnas 
   byrow = TRUE)        # completar matriz por filas 
...
Cuando construimos una matriz directamente con elementos de datos, el contenido de la matriz se llena a lo largo de la orientación de la columna de forma predeterminada. 

Por ejemplo, en el siguiente fragmento de código, el contenido de B se completa a lo largo de las columnas de forma consecutiva.
```{r}
B = matrix(
 c(2, 4, 3, 1, 5, 7),
 nrow=3,
...
# de aa a ta
# no queremos que cambie la lista ubicada en el índice 2, sino que cambie el primer elemento del vector ubicado en el índice 2.
#cambia los elementos 1 y 2 del vector que está ubicado en el índice 2.
...
```{r}
# estamos pidiendo cambiar el elemento 1 de la lista ubicada en el índice 2 (no del vector)
y[2][1] = "ta" 
...
* **Vector de columna de data frame**

Hacemos referencia a una columna de data frame con el operador de doble corchete "[[]]".

Por ejemplo, para recuperar el vector de la novena columna del conjunto de datos incorporado mtcars, escribimos mtcars[[9]].

```{r}
mtcars[[9]]
```

Podemos recuperar el mismo vector columna por su nombre.

```{r}
mtcars[["am"]]
```

También podemos recuperar con el operador "$" en lugar del operador de doble corchete.

```{r}
mtcars$am
```
.
Otra forma más de recuperar el mismo vector de columna es utilizar el operador de corchete único "[]". Anteponemos el nombre de la columna con un carácter de coma, lo que indica una coincidencia de comodín para la posición de la fila.
```{r}
mtcars[,"am"]
```

* **Sector de columna de data frame**

Recuperamos un segmento de columna de data frame con el operador de corchete simple "[]".

- Indexación numérica

El siguiente es un segmento que contiene la primera columna del conjunto de datos incorporado mtcars.
```{r}
mtcars[[1]]
```
...
Podemos recuperar el mismo segmento de columna por su nombre.
```{r}
mtcars["mpg"]
```

Para recuperar un segmento de data frame con las dos columnas mpg y hp, empaquetamos los nombres de las columnas en un vector de índice dentro del operador de corchete único.

```{r}
mtcars[c("mpg", "hp")]
```

* **Sector de fila de marco de datos**

Recuperamos filas de un marco de datos con el operador de corchete único, tal como lo hicimos con las columnas. Sin embargo, además de un vector de índice de posiciones de fila, agregamos un carácter de coma adicional. Esto es importante, ya que la coma adicional indica una coincidencia de comodines para la segunda coordenada para las posiciones de las columnas.

- Indexación numérica

Por ejemplo, lo siguiente recupera un registro de fila del conjunto de datos integrado mtcars. Tenga en cuenta la coma adicional en el operador de corchetes, y no es un error tipográfico. Establece que el Camaro Z28 de 1974 tiene un rendimiento de gasolina de 13.3 millas por galón y un motor de ocho cilindros y 245 caballos de fuerza, ..., etc.

```{r}
mtcars[24,]
```

Para recuperar más de una fila, usamos un vector de índice numérico.
```{r}
mtcars[c(3, 24),]
```
...
Podemos recuperar una fila por su nombre.
```{r}
mtcars["Camaro Z28",]
```
...
Y podemos empaquetar los nombres de las filas en un vector de índice para recuperar varias filas.
```{r}
...
```{r}
orden = seq(1, 8, by=2) #seq - significa secuencia
data1 = data.frame(carreras , estudiantes, orden)
...
Existen multiples paquetes con funciones que permiten la importacion de datos desde diversas fuentes de informacion. Sin embargo, en esta clase vamos a hacer uso del paquete **rio**.
...
Aquí está el valor de celda de la primera fila, segunda columna de estadistica.
...
Además, podemos usar los nombres de fila y columna en lugar de las coordenadas numéricas.
```{r}
#row.names(estadistica) = estadistica$Marca.temporal
```

```{r}
#comodin
estadistica[, "Â.CuÃ.l.es.tu.edad."]
```
...
Las dimensiones de un data frame se ven...
```{r}
dim(estadistica)
```
